...
export async function getStaticProps() {
  ...
  const [instruments, storyFetchResult, teachers, likesList] = await Promise.all([
    ...
    fetchStoryData(StoryblokService, language, 'music-lessons/piano-lessons'),
    fetchTeachersData(getTeachers, 'piano'),
    likes.search('piano'),
  ]);
...
function PianoLessonsPage({ story, language, instrument, instruments, _online, recommendations }) {
  ...
  useEffect(() => {
    const loadInstrumentLocations = async () => {
      ...
      if (data) setInstrumentLocations(transformInstrumentLocations(data, { oldInstrument:'piano', newInstrument:'piano' }));
    ...
    >
      ...
      <StructuredData
        data={{
          ...
          '@type': 'FAQPage',
          mainEntity: [
            { '@type': 'Question', name: 'How do I know that the piano teachers are qualified?', acceptedAnswer: { '@type':'Answer', text:'All Matchspace Music piano teachers are checked by a qualified music teacher and activated on our platform.' }},
            { '@type': 'Question', name: 'How much do piano lessons cost?', acceptedAnswer: { '@type':'Answer', text:'The cost of lessons varies from canton to canton and starts at around CHF 85 to CHF 115 per hour. A subscription with 5 lessons costs around CHF 340 for children and around CHF 425 for adults.' }},
            { '@type': 'Question', name: 'From what age can you learn to play the piano?', acceptedAnswer: { '@type':'Answer', text:'Children can learn to play the piano from around 4 years old, depending on their stage of development. Piano lessons are also ideal for teenagers, adults and senior citizens up to an advanced age.' }},
            { '@type': 'Question', name: 'Where do piano lessons take place?', acceptedAnswer: { '@type':'Answer', text:"Our piano teachers offer their lessons in their studio, as a home visit, i.e. at the piano student's home, or online lessons if required." }},
            { '@type': 'Question', name: 'How do I find the right piano teacher?', acceptedAnswer: { '@type':'Answer', text:'Hundreds of qualified piano teachers teach at Matchspace Music. Search for the instrument in your area and choose the teacher that suits your needs. Get in touch and arrange your first lesson.' }},
          ]
        }}
      />
    </Layout>
