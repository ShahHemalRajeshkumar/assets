...
export async function getStaticProps() {
  ...
  const [instruments, storyFetchResult, teachers, likesList] = await Promise.all([
    fetchInstrumentsData(instrumentsIndex),
    fetchStoryData(StoryblokService, language, 'music-lessons/singing-lessons'),
    fetchTeachersData(getTeachers, 'singing'),
    likes.search('singing'),
  ]);
  const instrument = getInstrumentById(instruments, 'singing', 'en') || null;
  const story = storyFetchResult.result?.data?.story || null;
  return {
    props: {
      recommendations: likesList?.hits || [],
      instrument,
      instruments,
...
export async function getStaticPaths() {
  return {
    paths: [{ params: { language: 'ch-en' } }, { params: { language: 'ch-de' } }],
    fallback: 'blocking',
  };
}
...
function SingingLessonsPage({ story, language, instrument, instruments, _online, recommendations }) {
  ...
  useEffect(() => {
    const loadInstrumentLocations = async () => {
      ...
      const transformedData = transformInstrumentLocations(data, {
        oldInstrument: 'piano',
      ...
      if (data) setInstrumentLocations(transformedData);
    ...
    >
      ...
      <StructuredData
        data={{
          ...
          '@type': 'FAQPage',
          mainEntity: [
            { '@type': 'Question', name: 'How do I know that the singing teachers are qualified?', acceptedAnswer: { '@type': 'Answer', text:'All Matchspace Music singing teachers are checked by a qualified music teacher and activated on our platform.' }},
            { '@type': 'Question', name: 'How much do singing lessons cost?', acceptedAnswer: { '@type': 'Answer', text:'The cost of lessons varies from canton to canton and starts at around CHF 85 to CHF 115 per hour. A subscription with 5 lessons costs around CHF 340 for children and around CHF 425 for adults.' }},
            { '@type': 'Question', name: 'From what age can you learn to sing?', acceptedAnswer: { '@type': 'Answer', text:'Children can learn to sing from around 8 - 12 years old, depending on their stage of development. Singing lessons are also excellent for teenagers, adults and seniors up to an advanced age.' }},
            { '@type': 'Question', name: 'Where do the singing lessons take place?', acceptedAnswer: { '@type': 'Answer', text:"Our singing teachers offer their lessons in their studio, as a home visit, i.e. at the student's home, or online lessons if required." }},
            { '@type': 'Question', name: 'How do I find the right singing teacher?', acceptedAnswer: { '@type':'Answer', text:'Hundreds of qualified singing teachers teach at Matchspace Music. Search for the instrument in your area and choose the teacher that suits your needs. Get in touch and arrange your first lesson.' }},
          ]
        }}
      />
    </Layout>
